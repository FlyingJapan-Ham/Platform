...
- 정적 파일을 브라우저로 열면 폼 아래에 **정적 HTML 모드** 안내가 나타납니다. 조회 버튼을 누르면 입력한 PHP 주소(기본값 `http://127.0.0.1:8000/orders_today.php`)로 이동하여 같은 파라미터로 다시 조회합니다. 주소는 로컬 스토리지에 저장되므로 한 번만 입력하면 됩니다.
